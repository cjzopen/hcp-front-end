...
<script type="text/javascript">
	...
	$().ready(function() {
		...
		$(".stripe_tb tr").click(function() {
			if (old_tr)
				old_tr.removeClass("click");
			$(this).addClass("click");
			old_tr = $(this);
			select_row_idx = $(this).attr('line_no');
			$('#dept_id' + select_row_idx).attr('checked', true);
		});

		$(".stripe_tb tr").dblclick(function() {
			closeLOV();
		});
		document.title = 'List Of Values';
	});
...
<div style="padding:10px; margin-bottom:10px;" class="ui-widget-content ui-corner-all">
		...
		?>
			...
			<td> <input class="control_checkbox" type="radio" name="choose_rdo" id="dept_id<?php echo $i; ?>" value="<?php echo $result['dept_id'].'||'.$result['dept_no'].'||'.$result['dept_name']; ?>" ontouchstart="test2(this);" ></td>
						<td>
...
function test2(item)
{
	 ...
	 try{
		...
		/* var dept_id = document.getElementById('dept_id2').value;
				 var dept_no = document.getElementById('dept_no2').value;
				 var dept_name = document.getElementById('dept_name2').value;
				 sessionStorage.setItem('agent_id',dept_id );
			sessionStorage.setItem('agent_code', dept_no ) ;
			sessionStorage.setItem('agent',dept_name);*/
 ...
 /*$("input:radio").click(function() {
	 document.getElementById('test').innerHTML="radio click";
   try{
   //var count  = document.getElementById("count").value;
				...
				// var dept_name = document.getElementById('dept_name2');	
					//if ( count == 1) select_row_idx = 0;
				var aaa=  $(this).val().split('||');
