...
class Abs_approve extends CI_Controller {
 ...
 function index()
 {
   //$this->load->helper(array('form'));
    ...
    $rowCount = $this->AresApprove_Model->GetApproveInfo($_SESSION ['user'] ['company_id'],$_SESSION ['user']['emp_seq_no'],'absence' ,'','');
   
	// 分頁設置
	$config['base_url'] = site_url("abs_approve/index"); //url地址
	$route['abs_approve/index/(:num).html'] = "abs_approve/index";
    ...
    $config['first_url'] = site_url('abs_approve'); // 要設置的第一頁鏈結
	...
	$this->pagination->initialize($config); 
     ...
     $data['list'] = $this->AresApprove_Model->GetApproveInfo($_SESSION ['user'] ['company_id'],$_SESSION ['user']['emp_seq_no'],'absence' ,$page,$config['per_page']);
	//$data['list'] = $result; // 獲取資料庫裏的資料
     // var_dump($result );exit; 
	// 生成分頁鏈結
	$data['links'] = $this->pagination->create_links();
    $data['scriptname'] ='leave_approve';
	...
	{
		$this->load->view('block_leave_approve_list_view', $data);
	}
	else
	{
	 ...
	 $this->load->view('block_leave_approve_result_view',$data);
	}
